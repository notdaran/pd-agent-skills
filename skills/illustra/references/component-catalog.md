...
| funnel-chart | sales-funnel set: five-steps (full), two-periods (x2 mini) | **3rd draw** | SVG conversion funnel: N bottom-anchored proportional bars + gradient slope trapezoids between them, optional drop annotation (level line + measured arrow + delta label). Drawn 3x in one run (full 628x350, mini 256x74 accent, mini 120x45 grey). **Promotion PROPOSED** (pending member) - params: stage heights array, bar/slope colour, optional annotated gap index |
| lane-split-panel | sales-funnel: three-ways | no | white "share of one total" read-out: header + count, one stacked summary bar, then N lanes of [name + count | big % | proportional track]. Generic split-of-total device (checkout paths, traffic sources, device mix). Hold (member) |
| leak-list-panel | sales-funnel: single-page | no | white list: title + count badge + rows of [page name | meta line | red delta]. Same chrome as the promoted `check-row-panel` with a delta value in place of the glyph - if drawn again, add it as a **check-row-panel variant**, not a new part (same call as `funnel-bars`) |
| express-wallet-row | sales-funnel: three-ways | no | storefront express-checkout row: Shop Pay (accent) / Apple Pay (ink) / G Pay (outlined) pills under an "or check out with" rule. Real labels on a concept surface (R12). Reusable for any checkout-path card. Hold |
| device-split-card | sales-funnel: two-periods | no | small white card comparing one rate across Desktop / Mobile as two labelled tracks + values. Reusable "split by device" satellite. Hold |
| market-label | page-audit set (as `.mk`), localize set: all 5 | yes (3rd+) | small uppercase accent chip naming the market / element a page belongs to ("DEFAULT PAGE", "GERMANY VERSION"). Opaque variant required when it sits on a rule (R18); `.mute` grey variant marks the default/untouched side. Promotion PROPOSED (pending member) |
| version-text-field | localize: copy | no | white in-app text input - muted field label + locale chip + bordered input holding the REAL market string + typing caret. The "you write this string yourself" proof; sibling of ai-prompt-panel (that one is a describe->generate textarea, this is a single-field edit). Hold |
| photo-swatch-row | localize: images | no | row of photo tiles in the UI palette (`--concept-img` + tint variants, mountain glyph) with one carrying an accent ring + tick. Reads as "different pictures", deliberately NOT the grey `--placeholder-bg` empty slot (that means unfinished - see ghost-section / audit-unfinished). Hold |
| destination-row | localize: links-and-offers | no | link glyph + who ("Default page" / "This version") + the path, default struck-through and version in the accent. The "same button, different target" device. Hold |
| language-menu | localize-ai-translator, localize: language | YES (2nd) | white dropdown: dot rows + active check + "50+" count + an AI-translator footer row. Second draw -> promotion PROPOSED (pending member) |
| section-order-panel | localize: layout | no | reorder list: grip + label + status value (moved up / added here / removed struck), with a green insert line + "+" handle between rows. The Mode-A light-card counterpart of the existing `drag-insert` part (2nd appearance of that motif, in a list rather than on a canvas). Hold |
| offer-pill-swap | localize: images | no | old badge pill struck (`.pill.off` + line-through) -> arrow -> new badge pill in the accent (`.pill.on`). Generic "this value was replaced" device, reusable well beyond badges. Hold |
| string-kind-list | ai-translator: text-you-wrote | no | white panel of [icon tile | kind of string | count] rows. Structurally a `check-row-panel` with a periwinkle glyph TILE instead of a status glyph - if drawn a 3rd time make it a `check-row-panel` variant, not a new part. Hold |
| code-diff-panel | ai-translator: html-liquid | no | illustrative code block: structure tokens muted (`t-tag`/`t-attr`/`t-mark`), Liquid in an accent chip that passes through, and struck EN -> highlighted translation inline. The "only the words change" device; reusable for any pass-through/diff story. Promotion PROPOSED (pending member). NOT an app-UI screenshot sim - deliberately plain, no editor chrome |
| language-picker | localize (as language-menu), ai-translator: 50-languages | YES (2nd draw) | header + count, search field, rows with an accent dot on the chosen one, fade at the list tail, CTA. Second appearance of the loc set's `language-menu` -> strong promotion signal. Promotion PROPOSED (pending member) |
| kv-translation-rows | ai-translator: translate-and-adapt | no | [muted source string / strong translated string + origin chip] pairs in a white panel. Generic source->target mapping read-out. Hold |
| excluded-block | page-audit (as ghost-section), ai-translator: not-touched | YES (2nd draw) | dashed, desaturated block with an uppercase owner label = "this content is out of scope". Second appearance of the audit set's `ghost-section` idea -> promotion signal. Promotion PROPOSED (pending member) |
